...
# =============================
# PRODUCTION READINESS: Input Validation & Security
# =============================

# Critical: Validate PowerShell version (7.x required for modern features)
if ($PSVersionTable.PSVersion.Major -lt 7) {
  Write-Warning "PowerShell 7.x recommended for best experience. Current: $($PSVersionTable.PSVersion)"
}

# Critical: Validate competitor analysis inputs
if ($CompetitorAnalysis) {
  if (-not $CurrentBackupVendor) {
    throw "ERROR: -CompetitorAnalysis requires -CurrentBackupVendor (Commvault, Rubrik, or Other)"
  }
  if ($CurrentMonthlyCost -le 0) {
    throw "ERROR: -CurrentMonthlyCost must be greater than 0"
  }
  if ($ContractYearsRemaining -lt 0) {
    throw "ERROR: -ContractYearsRemaining cannot be negative"
  }
  Write-Host "✅ Competitor analysis enabled: $CurrentBackupVendor at `$ $CurrentMonthlyCost/month" -ForegroundColor Green
}

# Critical: Validate certificate auth requirements
if ($CertificateThumbprint -or $CertificateSubjectName) {
  if (-not $ClientId) {
    throw "ERROR: Certificate authentication requires -ClientId"
  }
  if (-not $TenantId) {
    throw "ERROR: Certificate authentication requires -TenantId"
  }
}

# Security: Warn about client secret usage (should be avoided in production)
if ($ClientSecret -and $UseAppAccess) {
  Write-Warning "⚠️  Client Secret authentication detected. Consider using certificate-based auth for production."
}
...
# =============================
# PRODUCTION READINESS: Enhanced Error Handling & Progress
# =============================

# Progress indicator function
function Write-ProgressIndicator([string]$Activity, [string]$Status, [int]$PercentComplete = -1) {
...
function Invoke-WithRetry {
  ...
  do {
    try {
      Write-ProgressIndicator -Activity $OperationName -Status "Attempt $attempt of $MaxRetries"
      & $ScriptBlock
      Write-ProgressIndicator -Activity $OperationName -Status "Completed successfully" -PercentComplete 100
      return
    }
    catch {
      ...
      if (-not $isLastAttempt) {
        ...
        Write-ProgressIndicator -Activity $OperationName -Status "Waiting ${DelaySeconds}s before retry..."
        Start-Sleep -Seconds $DelaySeconds
        $DelaySeconds *= 2  # Exponential backoff
      } else {
        Write-ProgressIndicator -Activity $OperationName -Status "Failed permanently" -PercentComplete 0
        throw "CRITICAL: $OperationName failed after $MaxRetries attempts. Last error: $($_.Exception.Message)"
      }
    }
...
# =============================
# PRODUCTION READINESS: One-Click Execution & Self-Healing
# =============================

# Auto-install missing modules (with user consent)
if ($SkipModuleInstall -eq $false) {
  $missingModules = @()
    ...
    if ($install -notin @('n', 'N')) {
      foreach ($m in $missingModules) {
        ...
        try {
          ...
          Write-Warning "Failed to install $m. You may need to run: Install-Module $m -Scope CurrentUser"
        }
      }
    }
  }
...
# Self-test: Validate script integrity
Write-ProgressIndicator -Activity "Initialization" -Status "Validating script integrity..."
try {
  $testAst = [System.Management.Automation.Language.Parser]::ParseFile($MyInvocation.MyCommand.Path, [ref]$null, [ref]$null)
  Write-Host "✅ Script syntax validation passed" -ForegroundColor Green
}
catch {
  throw "CRITICAL: Script contains syntax errors: $($_.Exception.Message)"
}

Write-ProgressIndicator -Activity "Initialization" -Status "Script ready for execution" -PercentComplete 100
...
if (-not $UseAppAccess) {
  ...
  if (Test-GraphSession -requiredScopes $baseScopes) {
    ...
    Write-ProgressIndicator -Activity "Authentication" -Status "Connecting to Microsoft Graph (delegated)..."
    ...
    if ($UseDeviceCode) {
      ...
      Write-Host "📱 Using Device Code Flow (no browser required)..." -ForegroundColor Cyan
    }

    Invoke-WithRetry -ScriptBlock {
      Connect-MgGraph @connectParams
    } -OperationName "Delegated Authentication"
  }

  # Self-check: confirm session includes scopes we asked for
  Invoke-WithRetry -ScriptBlock {
    Assert-Scopes -mustHaveScopes $baseScopes
  } -OperationName "Scope Validation"
} else {
...
# =============================
# Competitor Analysis (FIXED: Production-Ready Logic)
# =============================
if ($CompetitorAnalysis) {
  Write-Log "Performing competitor analysis..."

  # Competitor cost assumptions (market averages 2026)
  $competitorCosts = @{
    Commvault = @{
      BasePerUser = 25
      StoragePerGB = 0.15
  ...
  $competitor = if ($CurrentBackupVendor -and $competitorCosts.ContainsKey($CurrentBackupVendor)) {
    $competitorCosts[$CurrentBackupVendor]
  } else {
    $competitorCosts["Other"]  # Default assumptions
  }

  # FIXED: Use provided monthly cost as baseline (customer input)
  $providedMonthlyCost = $CurrentMonthlyCost
  $currentAnnualCost = $providedMonthlyCost * 12

  # Veeam MBS costs (FIXED: Realistic 2026 Azure storage pricing)
  # Hot tier: $0.018/GB/month, Cool: $0.01/GB/month, Archive: $0.0025/GB/month
  $veeamHotTierCost = ($MbsEstimateGB_Model * 0.6) * 0.018  # Assume 60% hot
  $veeamCoolTierCost = ($MbsEstimateGB_Model * 0.3) * 0.01   # Assume 30% cool
  $veeamArchiveTierCost = ($MbsEstimateGB_Model * 0.1) * 0.0025  # Assume 10% archive
  $veeamStorageCost = $veeamHotTierCost + $veeamCoolTierCost + $veeamArchiveTierCost

  # Veeam license: $0.75/user/month (2026 pricing)
  $veeamLicenseCost = $UsersToProtect * 0.75
  $veeamAnnualCost = ($veeamStorageCost + $veeamLicenseCost) * 12

  # ROI Analysis (FIXED: Realistic migration costs based on user count)
  $annualSavings = $currentAnnualCost - $veeamAnnualCost
  $roiYears = if ($ContractYearsRemaining -gt 0) { $ContractYearsRemaining } else { 5 }  # Default to 5 years
  $totalSavings = $annualSavings * $roiYears

  # Migration cost: $50 per user (realistic for M365 migrations)
  $migrationCostPerUser = 50
  $migrationCost = [Math]::Max($UsersToProtect * $migrationCostPerUser, 15000)  # Minimum $15K

  $netSavings = $totalSavings - $migrationCost
  $paybackMonths = if ($annualSavings -gt 0) { [Math]::Ceiling($migrationCost / ($annualSavings / 12)) } else { 0 }

  # Add competitor analysis to summary
  $summary | Add-Member -NotePropertyName CurrentVendor -NotePropertyValue $CurrentBackupVendor -Force
  $summary | Add-Member -NotePropertyName CurrentAnnualCost -NotePropertyValue ([Math]::Round($currentAnnualCost, 0)) -Force
  $summary | Add-Member -NotePropertyName VeeamAnnualCost -NotePropertyValue ([Math]::Round($veeamAnnualCost, 0)) -Force
  $summary | Add-Member -NotePropertyName AnnualSavings -NotePropertyValue ([Math]::Round($annualSavings, 0)) -Force
  $summary | Add-Member -NotePropertyName TotalSavings5Year -NotePropertyValue ([Math]::Round($totalSavings, 0)) -Force
  $summary | Add-Member -NotePropertyName NetSavingsAfterMigration -NotePropertyValue ([Math]::Round($netSavings, 0)) -Force
  $summary | Add-Member -NotePropertyName PaybackMonths -NotePropertyValue $paybackMonths -Force
  $summary | Add-Member -NotePropertyName MigrationComplexity -NotePropertyValue "Medium" -Force

  Write-Log "Competitor analysis complete: Current cost $([Math]::Round($currentAnnualCost, 0)), Veeam cost $([Math]::Round($veeamAnnualCost, 0))"
}
...
# =============================
# PRODUCTION READINESS: Final Validation & Success Metrics
# =============================
$scriptStartTime = Get-Variable -Name "scriptStartTime" -ValueOnly -ErrorAction SilentlyContinue
if (-not $scriptStartTime) { $scriptStartTime = Get-Date }

$executionTime = (Get-Date) - $scriptStartTime
$successMetrics = @{
  ScriptVersion = "2.1.0-Production"
  ExecutionTime = $executionTime
  OutputFiles = (Get-ChildItem $runFolder -File -ErrorAction SilentlyContinue).Count
  TotalSizeGB = [Math]::Round(($exBytes + $odBytes + $spBytes) / $GB, 2)
  HasCompetitorAnalysis = [bool]$CompetitorAnalysis
  AuthenticationMethod = if ($UseAppAccess) { "App-Only" } else { "Delegated" }
}

Write-Host "🎉 ASSESSMENT COMPLETE!" -ForegroundColor Green
Write-Host "📊 Results Summary:" -ForegroundColor Cyan
Write-Host "   • Users: $UsersToProtect" -ForegroundColor White
Write-Host "   • Total Data: $([Math]::Round($totalGB, 1)) GB" -ForegroundColor White
Write-Host "   • MBS Estimate: $([Math]::Round($mbsEstimateGB, 0)) GB" -ForegroundColor White
if ($CompetitorAnalysis) {
  Write-Host "   • Annual Savings: `$ $([Math]::Round($annualSavings, 0).ToString("N0"))" -ForegroundColor Green
}
Write-Host "   • Execution Time: $([Math]::Round($executionTime.TotalSeconds, 1))s" -ForegroundColor White
Write-Host "   • Output: $outHtml" -ForegroundColor White

Write-Log "Assessment completed successfully in $([Math]::Round($executionTime.TotalSeconds, 1)) seconds"
Write-Host ""
