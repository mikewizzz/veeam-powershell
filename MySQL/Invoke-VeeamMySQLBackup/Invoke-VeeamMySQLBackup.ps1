...
.DESCRIPTION
    Invoke-VeeamMySQLBackup is a Veeam Solutions Engineer tool that automates
    the deployment of MySQL hot backup integration for Veeam Backup & Replication.

    This script solves the competitive gap where Dell EMC NetWorker has a built-in
    MySQL Enterprise Backup (MEB) integration. Veeam achieves the same (or better)
    result using application-aware processing with pre-freeze/post-thaw scripts.
...
.NOTES
    Author:         Veeam Solutions Engineering
    Version:        1.0.0
    Requires:       PowerShell 5.1+
    Veeam Modules:  Veeam.Backup.PowerShell (optional, for job configuration)
    SSH Access:     Required for Linux server deployment

    Competitive Context (NetWorker):
    NetWorker's MySQL Enterprise Backup integration is a compiled module that
    invokes MEB via the nsrmysqld daemon. This Veeam solution provides:
      - Same MEB integration capability via pre-freeze scripts
      - Additional XtraBackup support (free alternative to MEB)
      - Universal FTWRL fallback for any MySQL edition
      - Full visibility into backup logic (open-source scripts vs. black box)
      - No additional NetWorker license cost for the MySQL module
#>
...
<body>
            ...
            <div class="method-comparison">
                <div class="method-card">
                    ...
                    <ul>
                        ...
                        <li>Same tool NetWorker uses internally</li>
                    </ul>
                </div>
                <div class="method-card">
    ...
    <div class="footer">
        <p>Generated by Invoke-VeeamMySQLBackup v${Script:VERSION} | Veeam Solutions Engineering</p>
    </div>
</body>
</html>
"@
