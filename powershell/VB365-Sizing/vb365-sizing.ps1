...
function Annualize-GrowthPct {
    ...
    $rounded = [math]::Round($pct, 4)
    if ($rounded -eq 0) { return 0.0 }
    return $rounded
}
...
function Invoke-DirectoryInventory {
    ...
    # Enabled member users (userType eq 'Member' or unset, accountEnabled eq true)
    try {
        $uri = "https://graph.microsoft.com/v1.0/users?`$filter=accountEnabled eq true and (userType eq 'Member' or userType eq null)&`$top=1&`$count=true"
        $resp = Invoke-Graph -Uri $uri -Headers @{ "ConsistencyLevel" = "eventual" }
        $script:enabledMemberCount = if ($resp.'@odata.count') { [int]$resp.'@odata.count' } else { "unknown" }
    } catch {
        $script:enabledMemberCount = "unknown"
    }

    Write-Log "Directory: Users=$($script:userCount), Licensed=$($script:licensedUserCount), Guests=$($script:guestUserCount), EnabledMembers=$($script:enabledMemberCount), Groups=$($script:groupCount), Apps=$($script:appRegCount), SPNs=$($script:spnCount), CAPolicies=$($script:caPolicyCount)"
...
"@)
  ...
  <details class="section">
    ...
    <div class="identity-kpi-grid">
      ...
      <div class="identity-kpi">
        <div class="identity-kpi-value">$(Format-CountValue $script:enabledMemberCount)</div>
        <div class="identity-kpi-label">Enabled Member Users</div>
      </div>
      <div class="identity-kpi">
...
function Export-SummaryData {
    $summaryProps = [ordered]@{
        ...
        Dir_EnabledMembers     = $script:enabledMemberCount
